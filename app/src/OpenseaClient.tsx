import { Requester, RestMethod, stringListFromString } from "@kibalabs/core";
...
export class OpenseaClient {
  ...
  public getTokenListings = async (registryAddress: string, tokenIds: string[]): Promise<Record<string, TokenListing | null>> => {
    ...
    for (let tokenIdChunksIndex=0; tokenIdChunksIndex < tokenIdChunks.length; tokenIdChunksIndex++) {
  ...
  public getTokenListing = async (registryAddress: string, tokenId: string): Promise<TokenListing | null> => {
    const queryData: Record<string, string | boolean> = {
      'token_ids': tokenId,
      'asset_contract_address': registryAddress,
      'include_orders': true,
    };
    const url = 'https://api.opensea.io/api/v1/assets';
    const response = await this.requester.makeRequest(RestMethod.GET, url, queryData);
    const asset = JSON.parse(response.content)['assets'][0];
    let listings: TokenListing[] = [];
    (asset['sell_orders'] || []).forEach((sellOrder: Record<string, unknown>): void => {
      if (sellOrder["side"] !== 1 || sellOrder["sale_kind"] !== 0 || sellOrder["cancelled"]) {
        return;
      }
      listings.push(new TokenListing(
    ...
    (asset['seaport_sell_orders'] || []).forEach((seaportSellOrder: Record<string, unknown>): void => {
      if (seaportSellOrder["side"] !== 'ask' || seaportSellOrder["order_type"] !== 'basic' || seaportSellOrder["cancelled"]) {
        return;
      }
      listings.push(new TokenListing(
